...
async def process(
    ...
    fail_at: Optional[str] = Query(None, description="Target service to fail: service-a, service-b, service-c"),
    ...
    try:
        ...
        if response.status_code != 200:
            ...
            raise HTTPException(status_code=502, detail=f"{SERVICE_NAME} received error from service-b: {error_detail}")
        ...
        duration_ms = round((time.time() - start_time) * 1000, 2)
        await send_log(trace_id, SERVICE_NAME, "ERROR", error_type="timeout", message="downstream service-b timed out", duration_ms=duration_ms)
        raise HTTPException(status_code=504, detail=f"{SERVICE_NAME}: downstream service-b timed out")
    except httpx.ConnectError:
        duration_ms = round((time.time() - start_time) * 1000, 2)
        await send_log(trace_id, SERVICE_NAME, "ERROR", error_type="connection_error", message="service-b is unavailable", duration_ms=duration_ms)
        raise HTTPException(status_code=503, detail=f"{SERVICE_NAME}: service-b is unavailable")
